...
export default function RegisterForm() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Registratie succesvol! Controleer je e-mail om je account te verifiëren.');
      router.push('/login');
    } catch (error) {
      toast.error('Er is een fout opgetreden tijdens het registreren.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-white">Registreren</h2>
